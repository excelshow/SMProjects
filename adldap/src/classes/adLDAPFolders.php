...
class adLDAPFolders {
    ...
    * @return bool
    */
    public function rename($ouname,$ounameNew, $container)
    {
        if (!$this->adldap->getLdapBind()) { return false; }
        if ($ouname === null) { return "Missing compulsory field [$ouname]"; }
    ...
    */
    public function listing($folderName = NULL, $dnType = adLDAP::ADLDAP_FOLDER, $recursive = NULL, $type = NULL) 
    {
        ...
        if ($recursive === true) {
            $sr = @ldap_search($this->adldap->getLdapConnection(), $searchOu, $filter, array('objectclass', 'distinguishedname', 'samaccountname'),$attrsonly = null, $sizelimit = 5000);
    ...
    */
    public function create($attributes)
    {
        ...
        $containers = "OU=" . implode(",OU=", $attributes["container"]);
        $result = @ldap_add($this->adldap->getLdapConnection(), "OU=" . $add["OU"] . ", " . $containers .", ". $this->adldap->getBaseDn(), $add); // lizd11 20121025
        if ($result != true) { 
            return false; 
        }
