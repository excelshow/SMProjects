//弹出创建{$APPINFO['APP_CNNAME']}窗口
function create_album(uid){
	ymPrompt.win({message:APP+'/Manage/create_album_ajax/uid/'+uid,width:340,height:230,title:'创建相册',iframe:true});
}
//ajax执行创建{$APPINFO['APP_CNNAME']}操作
function ajax_submit_create_album(){

	var	ran			=	Math.random();
	var name		=	document.create_album_form.name.value;
        var typeId              =       document.create_album_form.typeId.value;
        var info              =       document.create_album_form.info.value;
	...
	$.post(APP+'/Manage/do_create_album',{ajax:1,name:name,typeId:typeId,info:info,privacy:privacy,privacy_data:password,share:share,ran:ran},function(data){
	    if(data){
			//设置数据
