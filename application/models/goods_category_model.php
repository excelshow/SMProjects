...
class Goods_category_model extends CI_Model {
    ...
    function get_all() {
        $records = array();
        $query = $this->db->query('SELECT * FROM goods_category ORDER BY class_sequence');
    ...
    function get_child_num($classid) {
        $query = $this->db->query("select count(*) as children from goods_category where parent_id='$classid'");
        foreach ($query->result() as $row) {
            return $row->children;
        }
    }
    ...
    function move_category($from, $to) {
        ...
        $this->db->update('goods_category', $data);
        return true;
    }

    function insert($data) {
        if ($this->db->insert('goods_category', $data)) {
            return "ok";
        } else {
            return false;
        }
    ...
    function update($classid, $data) {
        ...
        //$this->db->update('goods_category', $data);
        if ($this->db->update('goods_category', $data)) {
            return "ok";
        } else {
            return false;
        }
