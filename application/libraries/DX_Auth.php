...
class DX_Auth {
    ...
    /// lizd11 chick uri
    function check_controller_permissions($controller,$allow = TRUE) {
        // First check if user already logged in or not
        if ($this->is_logged_in()) {
            // If user is not admin
            if (!$this->is_admin() && $controller) {
 ...
 /// lizd11 chick controller for system 
    function check_controller_permissions_system($controller,$role_id,$allow = TRUE) {
        // First check if user already logged in or not
        if ($this->is_logged_in()) {
            // If user is not admin
            if (!$this->is_admin() && $controller) {
                ...
                // Get URI permissions from role and all parents
                // Note: URI permissions is saved in 'uri' key
                $roles_allowed_uris = $this->get_permissions_value_system('uri',$role_id);
    ...
    // lizd11 
   function get_permissions_value_system($key,$role_id, $array_key = 'default') {
        $result = array();
        $rolerow = $this->_get_role_data($role_id);
        $role_id = $rolerow->DX_role_id;
        $role_name = $rolerow->DX_role_name;

        $parent_roles_id = $rolerow->DX_parent_roles_id;
        $parent_roles_name =$rolerow->DX_parent_roles_name;
        /*$role_id = $this->ci->session->userdata('DX_role_id');
        $role_name = $this->ci->session->userdata('DX_role_name');
