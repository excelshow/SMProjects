...
//          $this->cismarty->display($this->sysconfig_model->templates() . '/temp/index.tpl');
    ...
    function sms() {
        ...
        foreach ($system->result() as $row) {
            ...
            $this->db->where("sms_number = '" . $sms_number . "' and sm_status = 1");
            //  $this->db->update('staff_sms',$data);
    ...
    function staffAt() {
        ...
        foreach ($system->result() as $row) {
            ...
            if ($staff) {
                ...
                $this->db->where('tel', $row->tel);
                $this->db->update('staff_main_temp_address', $data);
            }
        }
    }
        ...
        foreach ($system->result() as $row) {
             if ($row->sa_tel) {
                    ...
                    if ($code == '873') {
                     ...
                     // $data['sa_email'] = $row->itname.'semir';
                      $this->db->where('sa_tel', $row->sa_tel);
...
//                $this->db->where('itname', $row->itname);
    ...
    function staffTemp() {
        $system = $this->db->query(" SELECT  * FROM staff_temp");
        foreach ($system->result() as $row) {

            $itname = explode('@', $row->itname);
            // print_r($row);
            // exit();
            if ($row->pop == 0 && $row->waifa == 0) {
                echo "///////////////////<br>";
            } else {
                $sys = $this->db->query(" SELECT  * FROM staff_main where itname = '" . $itname[0] . "'");
                $info = $sys->row();
                if ($info) {
                    // print_r($info);
                    if ($info->system_id) {
                        ...
                        //  print_r($row);
                        if ($row->pop == 1) {
                            if (in_array('42', $sIdarr)) {
                                
                            } else {
                                echo $row->itname;
                                array_push($sIdarr, '42');
                                //  print_r($sIdarr);
                            }
                        }
                        if ($row->waifa == 1) {
                            if (in_array('41', $sIdarr)) {
                                
                            } else {
                                echo $row->itname;
                                array_push($sIdarr, '41');
                            }
                        }
                        $sid = implode(',', $sIdarr);
                        //$this->db->where('itname',$itname[0]);
                        //$data['system_id'] = $sid;
                        //  $this->db->update('staff_main', $data);
                        print_r($sIdarr);
                    } else {
