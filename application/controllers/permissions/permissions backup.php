...
class Permissions extends CI_Controller {
    ...
    function staff_system_list() {
        ...
        if ($data['stafftemp']) {
            foreach ($data['stafftemp'] as $row) {
                ...
                $smsIp = $this->sms_model->staff_sms_by("itname = '" . $row->itname . "' and sms_ip <> '' and sm_status =1 ");
                if ($smsIp) {
    ...
    function staff_systemedit_save() {
        ...
        //  exit();
        if (in_array("41", $systemp)) {
            $this->api_eyou->user_edit($result->itname, 'has_remote', 1);
        } else {
            $this->api_eyou->user_edit($result->itname, 'has_remote', 0);
        }
        if (in_array("42", $systemp)) {
            $this->api_eyou->user_edit($result->itname, 'has_pop', 1);
        } else {
            $this->api_eyou->user_edit($result->itname, 'has_pop', 0);
        }
        if ($data['system_id']) {
...
//  	     $this->db->update('staff_temp_renzi', $data);
    ...
    function dg_curl($key, $data) {
        $ch = curl_init();
        $timeout = 5;
        curl_setopt($ch, CURLOPT_URL, 'http://10.90.18.38:81/api/GetUserInfo/' . $data . '?loginname=' . $key);
