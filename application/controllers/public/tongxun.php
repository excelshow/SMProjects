...
class Tongxun extends CI_Controller {
    ...
    function index() {
        ...
        $this->cismarty->display($this->sysconfig_model->templates() . '/public/tongxun.tpl');
    }
    ...
    function searchStaff() {
        ...
        // $this->cismarty->assign("links", $data['links']);
        $this->cismarty->assign("showmenu", $this->showmenu());
        $this->cismarty->display($this->sysconfig_model->templates() . '/public/search_staff.tpl');
    }
    ...
    function sms() {
        ...
        //echo $where;
        $stafftemp =1; // $this->staff_model->get_staff_by($where2);
        // print_r($stafftemp);
        if ($stafftemp) {
            $where = ""; //"staff_sms.sm_status = 1";
            $where .= "staff_sms.itname = '" . $k . "' ";
            $where .= " or staff_sms.sms_number = '" . $k . "' ";
            ///////////////////////////////////////
            ////$where = "staff_sms.sm_status = 1";
...
//                    $this->sms_model->staff_sms_edit($dept);
    ...
    function doSearchStaff() {
        ...
        //echo $where;
        $data['stafftemp'] = $this->staff_model->get_staffs(0, 100, $where);
        //print_r($data['staffs']);
        // 读取用户AD状态
        if ($data['stafftemp']) {
            foreach ($data['stafftemp'] as $row) {
        ...
        // $this->load->view('staffLayout', $data);
        $this->cismarty->assign("data", $data['staffs']);
        $this->cismarty->display($this->sysconfig_model->templates() . '/public/staff_result.tpl');
    }
