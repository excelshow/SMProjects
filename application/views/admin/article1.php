    ...
    //<![CDATA[
        $(document).ready(function(){
				...
				<?php if($mode=="recycle"): ?>
					$("#recover").click(function(){
						...
						if(!confirm('确定要恢复吗？'))
							...
							$("form").attr('action','<?php echo site_url('admin/article/recover') ?>?q=<?php echo $q; ?>&v=<?php echo $v; ?>&mode=<?php echo $mode; ?>');
						}
					});
				<?php endif;?>
            ...
            $("form").submit(function(){
				if($("input[name='title']").val()==""||$("input[name='content']").val()==""||$("select[name='class_id']").val()==""){
					hiAlert('标题和内容不能为空');
					return false;	
					}
            ...
            $("#uploadify").uploadify({
        		'uploader'       : '<?php echo base_url()?>assets/javascript/uploadify/uploadify.swf',
        		'script'         : '<?php echo site_url('admin/attachment/upload') ?>',
        		'cancelImg'      : '<?php echo base_url()?>assets/javascript/uploadify/cancel.png',
        		'folder'         : 'attachments',
        		'queueID'        : 'fileQueue',
        		'auto'           : true,
        		'multi'          : true,
        		'onComplete'     : function(event,queueID,fileObj,response,data){
            								data = response.split('|');
            								if(data.length==3){
            									html = "<tr id='attach_"+data[0]+"'><td>"+fileObj.name+"</td><td>";
            									var arr = ['.jpg','.gif','.bmp','.png','.jpeg'];
            									if($.inArray(fileObj.type,arr)>=0){
													html += '<a href="#" onclick="tinyMCE.execCommand(\'mceInsertContent\',false,\'<img src=<?php echo site_url('attachments')."/" ;?>'+ data[1] + data[2] +'/>\');return false;">插入图片</a>';
													html += '<a href="#" onclick="make_cover('+ data[0] +');return false;" id="make_cover_'+data[0]+'">设为封面</a>';
													html += '<a style="display:none" href="#" onclick="cancel_cover('+ data[0] +');return false;" id="cancel_cover_'+data[0]+'">取消封面</a>';
	            								}else
													html += '<a href="#" onclick="tinyMCE.execCommand(\'mceInsertContent\',false,\'<a target=_blank href=<?php echo site_url('attachments')."/" ;?>'+ data[1] + data[2]  +'>'+ fileObj.name +'</a>\');return false;">插入文件</a>';
												html += '<a href="#" onclick="remove_attachment('+ data[0] +');return false;">删除附件</a>';
            									html +=  "</td></tr>";
            									$("#attachment").val($("#attachment").val()+data[0]+"s");
...
<form action="<?php echo site_url('admin/article/add')?>" method="post">
<table  class="treeTable">
   ...
   <!-- <tr><td>新闻来源</td><td><select name="article_from">
    ...
    <tr><td colspan="2"><input type="submit" name="submit" value=" 添加新闻 "  class="buttom" /></td></tr>
</table>
</form>
</div>
<?php endif;?>
