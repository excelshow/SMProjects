...
function PhpTreeView(Tname)
{
  if(typeof(Tname) != "string" || Tname == "")
    throw(new Error(-1, 'need Tname Object'));
...
PhpTreeView.prototype.browserCheck = function()
{
...
PhpTreeView.prototype.setStyle = function()
{

  var style = "<style>"+
  "DIV.PhpTreeView DIV IMG{border: 0px solid #FFFFFF;}"+
  "DIV.PhpTreeView DIV SPAN IMG{border: 0px solid #FFFFFF;}";
  if(this.wordLine)
  {
    style +="\
    DIV.PhpTreeView DIV\
...
PhpTreeView.prototype.atRootIsEmpty = function()
{
...
PhpTreeView.prototype.dataFormat = function()
{
  var a = new Array();
...
PhpTreeView.prototype.load = function(id)
{
...
PhpTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  this.index++;
...
PhpTreeView.prototype.getAttribute = function(source, name)
{
  var reg = new RegExp("(^|;|\\s)"+ name +"\\s*:\\s*([^;]*)(\\s|;|$)", "i");
  if (reg.test(source)) return RegExp.$2.replace(/[\x0f]/g, ";"); return "";
};

PhpTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(!isRoot)
  {
    ...
    node.childAppend = node.parentNode.childAppend + (AtEnd ? "a" : "b");
    if(this.wordLine)
    {
...
PhpTreeView.prototype.word2image = function(word)
{
  var str = "";
...
PhpTreeView.prototype.buildNode = function(id)
{
...
PhpTreeView.prototype.focusClientNode      = function(id)
{
...
PhpTreeView.prototype.focusLink= function(id)
{
  if(this.currentNode && this.currentNode.id==id) return;
  this.focusClientNode(id);
};

PhpTreeView.prototype.expand   = function(id, sureExpand)
{
...
PhpTreeView.prototype.nodeClick = function(id)
{
  ...
  {
    this.GetFullPath(id);
	parent.form1.deptpath.value = window.g;
    
  }
  else
  {
	parent.form1.deptpath.value = "";
...
PhpTreeView.prototype.GetFullPath = function(id)
{
    eval("window.g = new Object();window.g = \"\";");

    if(this.node[id].parentNode.parentId != 0)
    {
        //alert(this.node[id].text);
        this.GetFullPath(this.node[id].parentId);
    }

    window.g = window.g + this.node[id].text + "\\";


};

PhpTreeView.prototype.getPath= function(sourceId)
{
...
PhpTreeView.prototype.focus = function(sourceId, defer)
{
  if (!defer)
  {
  ...
  if(path[0]!="0")
  {
    alert("node "+ sourceId +" haven't suspend the invalid node\r\n"+
      "node id  = "+ path.join(this.divider));
    return;
  }
...
PhpTreeView.prototype.clickHandle = function(e)
{
...
PhpTreeView.prototype.dblClickHandle = function(e)
{
  e = window.event || e; e = e.srcElement || e.target;
...
PhpTreeView.prototype.upperNode = function()
{
...
PhpTreeView.prototype.lowerNode = function()
{
  if (!this.currentNode) this.currentNode = this.node["0"];
...
PhpTreeView.prototype.pervNode = function()
{
...
PhpTreeView.prototype.nextNode = function()
{
...
PhpTreeView.prototype.expandAll = function()
{
...
PhpTreeView.prototype.setIconPath  = function(path)
{
...
PhpTreeView.prototype.setURL     = function(url){this.url = url;};

PhpTreeView.prototype.setTarget  = function(target){this.target = target;};
