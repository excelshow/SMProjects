    ...
    //div高度大于屏幕高度把屏幕高度赋给div，并出现滚动条  
	 ...
	 $.ajax({
        ...
        $('input[name="searchBut"]').click(function(){
		 ...
		 $('button[name="staffadd"]').click(function(){
			 $('#deptform').attr("action","{%site_url('staff/staff/staffaddinfo')%}");
...
<div class="pad10">
  ...
  <div  class="pageTitleTop">用户管 理 &raquo; 基本信息 &raquo; </div>
   <div class="fright " style="background:#F7F7F7; padding:5px 20px 2px 20px;">
     {%if ($sysPermission["staff_add"] == 1)%}
     <button class="buttom" name="staffadd" type="button" value="">新增用户</button>
    
     {%/if%}
      {%if ($sysPermission["staff_move_batch"] == 1)%}
      <button class="buttom" name="moveBatch" type="button" value="">批量调岗</button>
      {%/if%}
