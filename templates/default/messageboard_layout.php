...
<script type="text/javascript">
	...
	function pageInit()
	{
		$('#info_con').xheditor({upLinkUrl:"upload.php",upLinkExt:"zip,rar,txt",upImgUrl:"<?php echo site_url('product/uploadHtmlPic') ?>",upImgExt:"jpg,jpeg,gif,png",tools:'full'});
                	...
                	$this = $(this).val();
					 ...
					 function handler(tp){
						 if(tp=='ok'){
							$.ajax({
							   type: "POST",
							   url: "<?php echo site_url('messageboard/del')?>",
							   data: "id="+$this,
								   ...
								   if(msg=="ok"){
									  // $("tr#"+n).remove();
									   ymPrompt.succeedInfo({message:'操作成功！请稍候, 正在刷新页面....'});
										...
										//window.location = "<?php echo site_url('info/') ?>";
            	...
            	$("input[name='submit']").click(function(){
						...
						<?php //if($mode=="recycle"):?>
							if(!confirm('确定要删除这些信息吗？'))
								...
								$("form").attr('action','<?php echo site_url('info/physical_del') ?>?q=<?php echo $q; ?>&v=<?php echo $v; ?>&mode=<?php echo $mode; ?>');
						<?php // endif;?>
            	});

				            	
            	$("select[name='class_id']").change(function(){
							window.location = "<?php echo site_url('info/view') ?>?q="+$(this).val()+"&v="+$("select[name='is_verified']").val()+"&mode=<?php echo $mode ?>";
            	});
            	$("select[name='is_verified']").change(function(){
					window.location = "<?php echo site_url('info/view') ?>?q="+$("select[name='class_id']").val()+"&v="+$(this).val()+"&mode=<?php echo $mode ?>";
    			});
...
<form action="<?php echo site_url('info/multi_del')?>?q=<?php echo $q; ?>&v=<?php echo $v; ?>&mode=<?php echo $mode; ?>" method="post">
