...
<script type="text/javascript">
    //<![CDATA[
        $(document).ready(function(){
            ...
            <?php if($action=="view"):?>
            	$('button[name="edit"]').click(function(){
						window.location = "<?php echo site_url('downgallery/edit_downgallery') ?>/" + $(this).val();
            	});
            	$('button[name="del"]').click(function(){
                	$this = $(this).val();
					 if(!confirm('确定要删除此条信息吗？'))
							...
							$.ajax({
							   type: "POST",
							   url: "<?php echo site_url('downgallery/physical_del')?>",
							   ...
							   success: function(msg){
								   if(msg=="ok"){
									   ...
									   $("tr#"+n).remove();
									   //Alert("删除成功");
										//window.location = "<?php echo site_url('downgallery/') ?>";
							...
							window.location = "<?php echo site_url('downgallery/view') ?>";
							//hiAlert("删除成功");
							return false;
						}
            	});
            	...
            	$("input[name='submit']").click(function(){
						...
						<?php //if($mode=="recycle"):?>
							if(!confirm('确定要删除这些新闻吗？'))
								...
								$("form").attr('action','<?php echo site_url('downgallery/physical_del') ?>?q=<?php echo $q; ?>&v=<?php echo $v; ?>&mode=<?php echo $mode; ?>');
						<?php // endif;?>
            	});
				...
				<?php if($mode=="recycle"): ?>
					$("#recover").click(function(){
						...
						if(!confirm('确定要恢复吗？'))
							...
							$("form").attr('action','<?php echo site_url('downgallery/recover') ?>?q=<?php echo $q; ?>&v=<?php echo $v; ?>&mode=<?php echo $mode; ?>');
						}
					});
				<?php endif;?>
        ...
        function remove_attachment(id){
        	$.ajax({
        		   type: "POST",
        		   url: "<?php echo site_url('attachment/del')?>",
        		   data: "id="+id,
        		   success: function(msg){
            		   if(msg=="ok"){
                		   $("#attach_"+id).remove();
        ...
        function make_cover(id,value){
        	$.ajax({
     		   type: "POST",
     		   url: "<?php echo site_url('attachment/cover')?>",
     		   data: "id="+id+"&value=1",
     		   success: function(msg){
      		   			if(msg=="ok"){
...
<?php if($action=="add") : ?>
<form action="<?php echo site_url('downgallery/add')?>" method="post">
...
<form action="<?php echo site_url('downgallery/edit')?>" method="post">
<table  class="treeTable">
...
<form action="<?php echo site_url('downgallery/multi_del')?>?q=<?php echo $q; ?>&v=<?php echo $v; ?>&mode=<?php echo $mode; ?>" method="post">
	<table  class="treeTable">
