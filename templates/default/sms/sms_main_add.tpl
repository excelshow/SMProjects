	...
	// //////////////////////////////////
	 ...
	 //form validator//////////////////////////////////////////////////////////////
	  ...
	  jQuery.validator.addMethod("sms_number", function(value, element) {   
			...
			"");
	 jQuery.validator.addMethod("sms_sapnumber", function(value, element) {   
			return this.optional(element) || /^([S]{1})?([SM]{2})?([BL]{2})?([M]{1})?([0-9]{12})$/.test(value);   
			},
			"");
	 $('#sms_number').keypress(function(event) {
				...
				$('#sms_number').blur(function() {
				$(this).val($(this).val().toUpperCase());
			});
	$('#sms_sapnumber').keypress(function(event) {
				var key = event.which;//event.keyCode
				if (key >= 97 && key <= 122) {//找到输入是小写字母的ascII码的范围
				event.preventDefault();//取消事件的默认行为
				$(this).val($(this).val() + String.fromCharCode(key - 32));//转换
				...
				$('#sms_sapnumber').blur(function() {
				$(this).val($(this).val().toUpperCase());
			});
	  var addjs = {
            rules: {
				sms_number:{required: true,sms_number:true,remote:{
					 
								url: "{%site_url('sms/sms/check_add')%}/"+$("#sms_number").val(),
								type: "post",
								async:false,
								dataType:"json", 
							//alert($("#useremail").html());
								data: {
									   number: function () {
                                 	   return $("#sms_number").val();
                               		 }
								} 
							}
					 },
			/*	sms_sapnumber:{required: true,sms_sapnumber:true,remote:{
					 
								url: "{%site_url('sms/sms/check_add')%}/"+$("#sms_sapnumber").val(),
								type: "post",
								async:false,
								dataType:"json", 
							//alert($("#useremail").html());
            ...
            messages: {
                sms_number:{required: "",sms_number: "",remote:"此编号已经存在"},
				//sms_sapnumber:{required: "",sms_sapnumber: "",remote:"此编号已经存在"},
			...
			submitHandler : function(){
					 ...
					 url = "{%site_url('sms/sms/sms_main_add_com')%}";
