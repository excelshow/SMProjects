		...
		var deptTo = {
            submitHandler:function(){
			  ...
			  // return false;
                $.ajax({
                    type: "POST",
                    url: "{%site_url('qyhdd/qyhdd/qyh_dept_sync_do')%}",
                    cache:false,
                    data: post_data,
					beforeSend: function(){
			...
			$("button[name='inTo']").click(function(){
				$.ajax({
						type: "POST",
						url: '{%site_url("qyhdd/qyhdd/qyh_dept_sync")%}',
						cache:false,
						data: 'id='+ $(this).val(),
						success: function(msg){
					   //  $("#ouShow").html(msg);
			...
			$("button[name='upDate']").click(function(){						// return false;
						  $.ajax({
							  type: "POST",
							  url: "{%site_url('qyhdd/qyhdd/qyh_dept_edit')%}",
							  cache:false,
							  data: 'sd_id='+ $(this).val(),
							    beforeSend: function(){
								  hiClose();
			...
			$("button[name='del']").click(function(){
				...
				hiConfirm('你确定从QQ服务器删除这个组织结构吗？',null,function(r){
					if(r){
						$.ajax({
							type: "POST",
							url: "{%site_url('qyhdd/qyhdd/qyh_dept_del')%}",
							cache:false,
		 ...
		 function postdnAjax(val){
            $.ajax({
                type: "POST",
                url: '{%site_url("qyhdd/qyhdd/qyh_dept_list")%}',
                cache:false,
                data: 'id='+val,
                success: function(msg){
                 $("#ouShow").html(msg);
        ...
        <tr id="{%$row->id%}">
          ...
          {%if ($sysPermission["qyh_dept"] == 1)%}
                      ...
                      {%if ($row->qyh_dept == 1)%}
                        ...
                        <button class="button" name="upDate" type="button" value="{%$row->id%}">更新到企业号</button>
                         <button class="buttonOt" name="del" type="button" value="{%$row->id%}">从企业号分离</button>
                      {%else%}
                      <button class="button" name="inTo" type="button" value="{%$row->id%}">同步到企业号</button>
