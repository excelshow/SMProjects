		...
		var deptTo = {
            submitHandler:function(){
			  ...
			  // return false;
                $.ajax({
                    type: "POST",
                    url: "{%site_url('qyhdd/qyhdd/dd_dept_sync_do')%}",
                    cache:false,
                    data: post_data,
					beforeSend: function(){
			...
			// into qq
			$("button[name='inTo']").click(function(){
				$.ajax({
						type: "POST",
						url: '{%site_url("qyhdd/qyhdd/dd_dept_sync")%}',
						cache:false,
						data: 'id='+ $(this).val(),
						success: function(msg){
					   //  $("#ouShow").html(msg);
							...
							$("#action").val('add'); 
							$("#addInfo").html(msg);  
							hiBox('#adddept','导入组织到钉钉系统',450,'','','.a_close'); 
							$('#hiAlertform').bind("invalid-form.validate").validate(deptTo); 
							//$("#adddept").show();   
			...
			// update qq
			$("button[name='upDate']").click(function(){						// return false;
						  $.ajax({
							  type: "POST",
							  url: "{%site_url('qyhdd/qyhdd/dd_dept_edit')%}",
							  cache:false,
							  data: 'sd_id='+ $(this).val(),
							    beforeSend: function(){
								  hiClose();
			...
			$("button[name='del']").click(function(){
				...
				hiConfirm('你确定从钉钉服务器删除这个组织结构吗？',null,function(r){
					if(r){
						$.ajax({
							type: "POST",
							url: "{%site_url('qyhdd/qyhdd/dd_dept_del')%}",
							cache:false,
		 ...
		 function postdnAjax(val){
            $.ajax({
                type: "POST",
                url: '{%site_url("qyhdd/qyhdd/dd_dept_list")%}',
                cache:false,
                data: 'id='+val,
                success: function(msg){
                 $("#ouShow").html(msg);
    ...
    <div id="ouShow" style=" " >
      <table  class="treeTable" id="treeTable">
        ...
        <tr id="{%$row->id%}">
          ...
          {%if ($sysPermission["qyh_dept"] == 1)%} 
                      {%if ($row->dd_dept == 1)%} 
                        <button class="button" name="upDate" type="button" value="{%$row->id%}">更新到钉钉</button>
                         <button class="buttonOt" name="del" type="button" value="{%$row->id%}">从钉钉分离</button>
                      {%else%}
                      <button class="button" name="inTo" type="button" value="{%$row->id%}">同步到钉钉</button> 
                      {%/if%} 
          {%/if%}</td>
        </tr>
        {%/foreach%}
