    ...
    //div高度大于屏幕高度把屏幕高度赋给div，并出现滚动条 
	 ...
	 $.ajax({
            ...
            success: function(msg){
                ...
                var outree = {"data": "Semir", state : "open", "attr":{"id":"0"},children:  msg};
                //alert(msg);
		...
		function loadstaff(val,key){
			 $.ajax({
                type: "POST",
                url: "{%site_url('permissions/permissions/staff_system_list')%}/"+val,
                cache:false,
        ...
        var edit = function(){
               $this = $(this).val();
                    $.ajax({
                        type: "POST",
                        url: "{%site_url('permissions/permissions/systemedit')%}",
                        data: "id="+$this,
                        success: function(msg){
                           //  alert(msg);
                             if(msg=="0"){ 
		 ...
		 var addjs = {
			...
			submitHandler : function(){
								...
								var url = "{%site_url('permissions/permissions/staff_systemedit_save')%}";
