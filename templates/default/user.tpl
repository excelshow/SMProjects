...
<script type="text/javascript">
    ...
    {
        ymPrompt.alert({title:'说明:'+aaa,message:value});
    }
</script> 
<script type="text/javascript">
    //<![CDATA[
...
<div class="pad10">
   <span class="addAdmin fright" id="addadmin" >添加管理员</span>
    ...
    <div id="adduser" style="display:none; " >
      <form id="new_user" method="post" action="{%site_url('system/user_add')%}">
        <fieldset>
    ...
    <div id="edituser" style="display:none; ">
      <form id="edit_user" method="post" action="{%site_url('system/user_edit')%}">
        <fieldset>
    ...
    <div id="editpass" style="display:none; ">
      <form id="edit_pass" method="post" action="{%site_url('system/user_editpass')%}">
        <fieldset>
    ...
    <div >
      <table id="treeTable" class="treeTable">
        ...
        <tr id="{%$row->uid%}">
          <td>{%$row->username%}</td>
          <td>{%$row->email%}</td>
          <td>{%$row->group_name%}<span style="padding:15px 0 0 10px; margin:0px;" ><a href="#" onclick="makes('{%$row->makes%}','{%$row->group_name%}')"><img src="{%$base_url%}templates/{%$web_template%}/images/onFocus.gif" border="0" stlye=""></a></span></td>
          <td><button class="edit" name="edit" type="button"
                                                value="{%$row->uid%}" ></button>
            &nbsp;&nbsp;&nbsp;
            <button class="editpass" name="editpass" type="button"
                                                value="{%$row->uid%}"></button>
            &nbsp;&nbsp;
            <button class="delete" name="del" type="button"
                                                value="{%$row->uid%}"></button></td>
        </tr>
        {%/foreach%}
      </table>
    </div>
